document.addEventListener("DOMContentLoaded", () => {
    inicializarGraficoConjuntos();
    inicializarGraficoBurbujas(); // <-- AQUÍ SE DISPARA TU NUEVO GRÁFICO
});

function inicializarGraficoConjuntos() {
    ...
    <p style="font-family: 'Georama', sans-serif; font-size: 0.8rem; color: #666; line-height: 1.5; margin-top: 14px; font-style: italic; border-top: 1px dashed #e0e0e0; padding-top: 8px;">
        <strong>* Nota de excepción:</strong> El diagrama refleja de forma estricta la exclusividad legal de las universidades para otorgar Grados Académicos en Chile. Se traza como excepción el programa de continuidad de estudios del IP Duoc UC (Convenio MAVA), el cual, al operar bajo el alero institucional de la Pontificia Universidad Católica de Chile, permite de forma anómala la articulación técnico-profesional directa hacia el grado de Licenciatura y el postgrado.
    </p>`;
    }
}

// ── Gráfico de Dispersión (Nubes de costos/tiempo) ──
        ...
        options: {
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        stepSize: 2,
                        callback: function (value) { return value + " sem."; }
                    }
                },
                y: {
                    grid: { color: "rgba(0,0,0,0.06)" },
...
// ── Gráfico de Burbujas (Masa Estudiantil) ──
function inicializarGraficoBurbujas() {
    ...
    const carrerasUniversidades = [
        ...
        { x: 55, y: 20, r: Math.sqrt(55) * 1.2, name: "INGENIERIA EN REALIDAD VIRTUAL Y DISEÑO DE JUEGOS DIGITALES", vacantes: 55 },
        ...
        { x: 35, y: 18, r: Math.sqrt(35) * 1.2, name: "COMUNICACION DIGITAL ESPECIALIDAD DISEÑO Y DESARROLLO DE VIDEOJUEGOS", vacantes: 35 },
    ...
    const carrerasIPCFT = [
        ...
        { x: 225, y: 35, r: Math.sqrt(225) * 1.2, name: "TECNICO DE NIVEL SUPERIOR EN DISEÑO Y MARKETING DIGITAL", vacantes: 225 },
        { x: 170, y: 75, r: Math.sqrt(170) * 1.2, name: "TECNICO DE NIVEL SUPERIOR EN DISEÑO Y ANIMACION DIGITAL", vacantes: 170 },
        ...
        { x: 45, y: 55, r: Math.sqrt(45) * 1.2, name: "TECNICO DE NIVEL SUPERIOR EN DISEÑO GRAFICO DIGITAL", vacantes: 45 },
        { x: 40, y: 22, r: Math.sqrt(40) * 1.2, name: "TECNICO EN DISEÑO Y CONTENIDO PARA PLATAFORMAS DIGITALES", vacantes: 40 }
    ];
        ...
        options: {
            ...
            plugins: {
                legend: {
                    display: true,
                    position: 'top',
                    labels: { font: { family: "'Georama', sans-serif", size: 12, weight: '500' }, usePointStyle: true, boxWidth: 10 }
                },
                tooltip: {
    ...
    fetch(URL_API)
        ...
        .then((objetoJSON) => {
            ...
            carreras.forEach((carrera) => {
                ...
                const claseFila = carrera.destacar == 1 ? 'class="fila-destacada"' : '';
                
                tablaSies.innerHTML += `
                    <tr ${claseFila}>
                        <td>${carrera.institucion}<small>${carrera.nombre_carrera}</small></td>
                        <td>${carrera.area_disciplinar}</td>
                        <td>
